...
public abstract class Backend {
    ...
    public final RegistrationResult register(@NotNull Tree in) {
        ...
        // check if we already have a tree with this ID in our system/'warm' storage.
        Tree current = trees.get(in.getID());

        if (current != null) {
            // case 1: tree is registered and being tracked by the system.
            // we will merge data that was provided by the input tree and return the already tracked tree we had stored.
            // caused by: loading data from a tree generated by the factory when that config is already loaded.
            ...
            if (load(in)) {
                // case 2a: this tree is not currently tracked, so we need to start tracking it.
                // if we successfully load data onto this tree (i.e. this tree exists in 'cold' storage), that data is loaded onto this tree
                // and we return.
                return new RegistrationResult(in, RegistrationResult.LOADED);
            } else {
                // case 2b: this tree is not currently tracked, and we have never seen it before.
                // we will save it to the cold storage and return it.
                save(in);
    ...
    public static final class RegistrationResult {
        ...
        /**
         * This tree has been seen before by the backend, and so data has been loaded into it from the backend storage.
         */
